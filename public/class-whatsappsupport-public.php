...
class WhatsAppSupport_Public {
	...
	public function get_settings() {
		...
		$global_settings = get_option( 'whatsappsupport' );
		...
		if ( is_array( $global_settings ) ) {
			...
			$post_settings = is_object( $post ) ? get_post_meta( $post->ID, '_whatsappsupport', true ) : '';
		...
		$this->settings = apply_filters( 'whatsappsupport_get_settings', $this->settings, $post );
	...
	public function enqueue_styles() {
		...
		if ( $this->settings['show'] ) {
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/whatsappsupport.css', array(), $this->version, 'all' );
		}
	...
	public function enqueue_scripts() {
		...
		if ( $this->settings['show'] ) {
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/whatsappsupport.js', array( 'jquery' ), $this->version, true );
		}
	...
	public function footer_html() {
		...
		if ( $this->settings['show'] ) {
		?>
		<div class="whatsappsupport whatsappsupport--<?php echo $this->settings['position']; ?>" data-settings="<?php echo esc_attr(json_encode($this->settings)); ?>">
			...
			<?php if ($this->settings['message_text']): ?>
				<div class="whatsappsupport__box">
					<header class="whatsappsupport__header">
						...
						<div class="whatsappsupport__close">&times;</div>
					</header>
					<div class="whatsappsupport__message"><?php echo $this->formated_message(); ?></div>
				</div>
			<?php endif; ?>
		</div>
		<?php
	...
	public function formated_message() {
		...
		$replacements = apply_filters( 'whatsappsupport_message_replacements', array(
	...
	public function formated_message_send( $string ) {
		...
		$replacements = apply_filters( 'whatsappsupport_message_send_replacements', array(
	...
	public function get_title() {
		...
		return apply_filters( 'whatsappsupport_get_title', $title );
